...
boton.addEventListener('click', () => {
    ...
    if (tareaTexto) {
        ...
        const nuevaTarea = {
            text: tareaTexto,
            fecha: fecha.value, // Puedes usar esta fecha si la necesitas más adelante
...
function render_tasks() {
    ...
    etxerakoLanak.forEach((elemento, index) => {
        ...
        nuevaTareaDiv.innerHTML = `
            <strong>${elemento.text}</strong><br>
            <em>Fecha: ${elemento.fecha}</em><br>
            <em>Asignatura: ${elemento.asignatura}</em>
        `;
